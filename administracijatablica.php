...
$baza = new baza();
$baza->spojiBaza();
/*$tablica = "<div class='table-responsive'><table id='ispisKorisnika' class='table table-striped table-bordered dataTable DTTT_selectable' ><caption >Dnevnik</caption><thead><tr><th>Opis</th><th>Vrijeme</th><th>Korisnik</th></thead><tbody>";
...
if(isset($_POST['btnGeneriraj'])){
    $baza = new baza();
    $baza->spojiBaza();
    $tablica = $_POST['brTablica'];
    $tablica1 = $_POST['brTablica'];
    $array = array(
    "korisnik" => 6,
    "log" => 4,
    "galerija" =>5,
    "kurirskasluzbapostar" => 2,
    "kurirska_sluzba" => 6,
    "mjesto" => 2,
    "mjesto_has_ruta" =>6,
     "paketPostar" => 10,
    ...
    $atribut = $array[$tablica];
   
    $tablica = "<div class='table-responsive'><table id='ispisKorisnika111' class='table table-striped table-bordered dataTable DTTT_selectable' ><caption >$tablica</caption><thead><tr>";
    //<th>Opis</th><th>Vrijeme</th><th>Korisnik</th></thead><tbody>";
    for ($i = 0; $i < $atribut+1; $i++){
        $tablica .= "<th> </th>";
    }
    $tablica .= "</tr></thead><tbody>";
    $upit = "SELECT * from ".$tablica1."";
    ...
    while ($red = $popisKorisnika->fetch_array()){
        ...
        $tablica .= "<th><button class='btn btn-success' data-id=$red[0] data-tablica=$tablica1 >Izmjeni</button></th>";
        $tablica .= "</tr>";
    //$tablica .= "<tr><td>{$red[1]}</td><td>{$red[2]}'><td>{$red[3]}</td></tr>";
    }
    }
...
<div class= "container">
	<div class = "row text-center">
        ...
        <select id="brTablica" name="brTablica">
          <option value="korisnik">Korisninci</option>
          <option value="log">Dnevnik</option>
          
          <option value="galerija">Galerija</option>
            <option value="kurirskasluzbapostar">Postar/Sluzba</option>
            <option value="kurirska_sluzba">Kurirska sluzba</option>
            <option value="mjesto">Mjesto</option>
            <option value="mjesto_has_ruta">Mjesto ruta</option>
            <option value="paketPostar">Paket postar</option>
             <option value="paketZahtjev">Paket zahtjev</option>
            <option value="ruta">Ruta</option>
            <option value="tippaket">Tip paket</option>
        </select>
              <button type ="submit" id="btnGeneriraj" name="btnGeneriraj" class = "btn btn-default" >Pretrazi</button><br/>
                </div>
		...
		<!---<div class ="modal-footer">
                   Ovde je gumb bio !!
                    ...
                    <div id="tablicaAdministracija" name ="tablicaAdministracija">
          <?php echo $tablica ?>
                    </div>
</div> 
 </div>      
